...
def test_mixins():
 ...
 #assert _d(1,2,3) == 2*fx
  ...
  ds = dill.source
  dd = dill.detect
  assert ds.getsource(dd.freevars(quafurniture)['f']) == '@quad_factory(a=0,b=4,c=0)\ndef quafurniture(x):\n  return x+1\n'
  assert ds.getsource(dd.freevars(quadruple)['f']) == '@doubler\ndef quadruple(x):\n  return 2*x\n'
  assert ds.importable(quafurniture, source=False) == 'from %s import quafurniture\n' % __name__
  ...
  result = ds.importable(quafurniture, source=True)
